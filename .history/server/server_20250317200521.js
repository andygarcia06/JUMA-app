...
app.get('/api/pending-companies', async (req, res) => {
  try {
    ...
    // 🔍 Charger les données de gestion de projet (ProjectManagement)
    let projectCompanies = await ProjectManagement.find();

    // 🔄 Fusionner les `members` depuis ProjectManagement + récupérer le `userId` (créateur)
    pendingCompanies = pendingCompanies.map(company => {
      const projectCompany = projectCompanies.find(proj => proj.id === company.id);

      const updatedCompany = {
        ...company.toObject(),
        members: projectCompany ? projectCompany.members : [], // Récupération des membres
        userId: company.userId // Ajout du créateur
      };

      // 📝 Log détaillé pour vérification
      console.log(`🔹 Entreprise : ${updatedCompany.companyName} (ID: ${updatedCompany.id})`);
      console.log(`   👤 Créateur : ${updatedCompany.userId}`);
      console.log(`   👥 Membres :`, updatedCompany.members);
