...
app.get('/api/suggestions/compareModules', (req, res) => {
  try {
    // Charger les fichiers JSON
    const modules = JSON.parse(fs.readFileSync(path.join(__dirname, 'modules.json')));
    const moduleTickets = JSON.parse(fs.readFileSync(path.join(__dirname, 'moduleTickets.json')));
    const tickets = JSON.parse(fs.readFileSync(path.join(__dirname, 'tickets.json')));

    // Extraire les détails des tickets
    const ticketDetails = tickets.map(ticket => ticket.detail.toLowerCase());

    // Fonction pour vérifier si un module est mentionné dans les détails des tickets
    const findMatchingModules = (moduleList) => {
      return moduleList.filter(module => {
        const moduleName = module.name.toLowerCase();
        return ticketDetails.some(detail => detail.includes(moduleName));
      });
    };

    // Trouver les modules correspondants
    const matchingModules = findMatchingModules(modules);
    const matchingModuleTickets = findMatchingModules(moduleTickets);
