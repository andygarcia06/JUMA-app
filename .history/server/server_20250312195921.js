...
app.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      console.log('[DEBUG] Utilisateur non trouvé avec ce username:', username);
    ...
    if (user.password !== password) {
      console.log('[DEBUG] Mot de passe incorrect pour utilisateur:', username);
      return res.status(401).json({ success: false, message: 'Mot de passe incorrect' });
    }

    const token = jwt.sign({ id: user._id, role: user.role }, process.env.JWT_SECRET || 'secretKey', { expiresIn: '1h' });

    console.log('[DEBUG] Connexion réussie pour:', username);
    ...
    console.error('[DEBUG] Erreur lors de la connexion :', err);
