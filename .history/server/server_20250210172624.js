...
app.get("/api/project-meteo/:ticketId", (req, res) => {
  ...
  console.log("📌 Contenu de allMessages:", JSON.stringify(allMessages, null, 2));
  ...
  const ticketCreatorId = ticket.userId; // Récupération du `userId` du créateur du ticket
  console.log("✅ Créateur du ticket:", ticketCreatorId);

  // 🔹 Récupérer les messages du bon ticket dans `messages.json`
  ...
  // 🔹 Extraire uniquement les messages écrits par le créateur du ticket
  console.log("📌 Créateur du ticket:", ticketCreatorId);
  console.log("📌 Nombre total de messages du ticket:", ticketMessages.messages.length);
  
  // ✅ Récupérer uniquement les messages écrits par le créateur du ticket
  ...
  console.log("📌 Détails des messages trouvés:", JSON.stringify(creatorMessages, null, 2));
  ...
  // 🔹 Comparer chaque message du créateur avec les entrées stockées dans la DB météo
  creatorMessages.forEach((message) => {
    Object.keys(dbEntries).forEach((category) => {
      dbEntries[category].forEach((entry) => {
        ...
        if (similarity > 0.5) { // Seuil de similarité ajustable
  ...
  const updateTicketsDB = (ticketId, meteo) => {
    const tickets = readJSONFile(DB_TICKETS_PATH);
    
    console.log("📌 Tickets chargés:", JSON.stringify(tickets, null, 2));
  
    const ticketIndex = tickets.findIndex(t => t.id === ticketId);
    console.log("📌 Index du ticket trouvé:", ticketIndex);
  
    if (ticketIndex === -1) {
      console.error("❌ Impossible de mettre à jour la météo, ticket non trouvé.");
      return;
    }
  
    // 🔹 Mise à jour de la météo du ticket
    tickets[ticketIndex].meteo = meteo;
  
    console.log("✅ Météo mise à jour dans tickets.json:", tickets[ticketIndex]);
  
    // 🔹 Sauvegarde du fichier `tickets.json`
    try {
      fs.writeFileSync(DB_TICKETS_PATH, JSON.stringify(tickets, null, 2), "utf-8");
      console.log(`✅ Météo ajoutée au ticket ${ticketId}: ${meteo}`);
    } catch (error) {
