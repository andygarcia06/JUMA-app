...
app.post('/tabs/:tabId/rows', async (req, res) => {
  try {
    ...
    // Extraction des données de la row depuis le body
    ...
    // Recherche du document DataCompanies
    const dataCompanies = await DataCompanies.findOne({ 'companies.id': companyId });
    if (!dataCompanies) {
      return res.status(404).json({ message: "Company not found" });
    }

    // Recherche de la compagnie dans le tableau companies
    ...
    // Création de la nouvelle row avec les données fournies
    const newRow = {
