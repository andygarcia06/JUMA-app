...
app.post('/upgrade-request', (req, res) => {
  ...
  // Mettre à jour l'utilisateur pour marquer qu'il a demandé un upgrade
  users[userIndex].upgradeRequested = true;
  console.log("[SERVER] Updated user:", users[userIndex]);
...
// Route GET pour récupérer les demandes d'upgrade
app.get('/upgrade-requests', (req, res) => {
  ...
  // Filtrer les utilisateurs ayant demandé un upgrade
  const upgradeRequests = users.filter(u => u.upgradeRequested === true);
  console.log("[SERVER] Upgrade requests:", upgradeRequests);
...
app.put('/update-role', (req, res) => {
  ...
  // Réinitialiser le flag upgradeRequested
