...
app.post('/api/users/:userId/update-progression', async (req, res) => {
  try {
    const { userId } = req.params; // Ici, userId doit être l'_id, par exemple "67d09cea89704d5dade1bd97"
    ...
    // Pour chaque module, on calcule la progression en se basant sur le nombre total de cours du module
    // et en filtrant les cours validés par l'utilisateur (user.courses)
    const modulesProgress = modules.map(module => {
      const totalCourses = module.courses.length; // Nombre total de cours dans le module
    ...
    // Recherche (ou création) du document UserProgress en se basant sur l'_id de l'utilisateur
