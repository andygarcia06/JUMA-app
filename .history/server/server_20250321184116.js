...
app.get('/api/users/:userId/progression', async (req, res) => {
  const { userId } = req.params; // Ici, userId représente le username (ex. "1" ou "ansy")
  console.log('[SERVER] Requête pour la progression de username:', userId);
  ...
  try {
    ...
    // Recherche l'entrée de validation des cours basée sur le username
    const userValidateEntry = await UserValidateCourse.findOne({ userId: user.username });
    const totalCourses = user.courses ? user.courses.length : 0;
    const validatedCourses = userValidateEntry ? userValidateEntry.validatedCourses : [];
    const validatedCourseCount = validatedCourses.length;
    ...
    console.log('[SERVER] UserValidateEntry:', userValidateEntry);
    console.log('[SERVER] Calcul de progression:', {
      totalCourses,
      validatedCourseCount,
...
// Mise à jour de la progression de l'utilisateur dans MongoDB (basée sur le username et les données des modules)
app.post('/api/users/:userId/update-progression', async (req, res) => {
  try {
    const { userId } = req.params; // Ici, userId représente le username (par exemple "1" ou "ansy")
    console.log('[SERVER] Requête update-progression pour username:', userId);
    ...
    // Pour chaque module, utiliser le nombre total de cours du module (et non la longueur de user.courses)
    const modulesProgress = modules.map(module => {
      const totalCourses = module.courses.length; // Nombre de cours dans ce module
      // Filtrer les cours de l'utilisateur qui appartiennent à ce module
    ...
    // Recherche (ou création) du document de progression dans UserProgress en se basant sur le username
