...
app.post('/validated-course', async (req, res) => {
  try {
    ...
    let userValidateEntry = await UserValidateCourse.findOne({ userId });

    if (!userValidateEntry) {
      userValidateEntry = new UserValidateCourse({ userId, validatedCourses: [] });
    }

    if (userValidateEntry.validatedCourses.includes(courseId)) {
      return res.status(400).json({ message: 'Ce cours a déjà été validé.' });
    }

    userValidateEntry.validatedCourses.push(courseId);
    await userValidateEntry.save();
...
// Route pour mettre à jour le nombre de réactions de l'utilisateurapp.post('/api/user/:userId/update-modification-count', async (req, res) => {
  const { userId } = req.params;
