...
app.get('/api/user/:userId/most-viewed-modules', async (req, res) => {
  try {
    const { userId } = req.params;
    console.log('[BACK] Requête pour les modules les plus consultés pour userId:', userId);

    // Rechercher la société via ProjectManagement
    const company = await ProjectManagement.findOne({ "members.userId": userId });
    if (!company) {
      console.log('[BACK] Aucun company trouvé pour cet utilisateur.');
    ...
    // Ici, nous supposons que le champ "company" dans Module correspond au companyName de la société.
    const modules = await Module.find({ company: company.companyName });
    console.log('[BACK] Nombre de modules récupérés:', modules.length);

    // Calculer le nombre total de réactions pour chaque module.
    const modulesWithReactions = modules.map(module => {
      ...
      return {
        id: module.id,
        title: module.title,
        courses: module.courses, // renvoi des cours pour affichage dans la modal
        reactions: totalReactions
      };
    });
    console.log('[BACK] Modules avec réactions (avant tri) :', modulesWithReactions);

    // Trier par nombre décroissant de réactions.
