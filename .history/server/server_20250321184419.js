...
// Mise à jour de la progression de l'utilisateur dans MongoDB (basée sur l'_id)
app.post('/api/users/:userId/update-progression', async (req, res) => {
  try {
    const { userId } = req.params; // Ici userId sera l'_id de l'utilisateur sous forme de chaîne hexadécimale de 24 caractères
    ...
    const modulesProgress = modules.map(module => {
      ...
      // Filtrer les cours validés par l'utilisateur qui appartiennent à ce module
    ...
    // Recherche (ou création) du document de progression en se basant sur l'_id de l'utilisateur
