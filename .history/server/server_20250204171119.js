...
// Définition du chemin du fichier JSON (assurez-vous que le chemin est correct)
const filePath = path.join(__dirname, './json/connectDatas.json');
console.log("[SERVER] filePath:", filePath);

//
// POST /upgrade-request
// Cette route reçoit une demande d'upgrade pour un utilisateur (en identifiant par username)
// et met à jour le flag upgradeRequested à true dans le fichier JSON.
//
app.post('/upgrade-request', (req, res) => {
  const { username } = req.body;
...
// Cette route renvoie la liste des utilisateurs qui ont demandé un upgrade.
// Seuls les utilisateurs dont le flag upgradeRequested est à true et dont le rôle est "utilisateur"
// seront retournés.
//
app.get('/upgrade-requests', (req, res) => {
  ...
  const upgradeRequests = users.filter(u => u.upgradeRequested === true && u.role === 'utilisateur');
...
//
// PUT /update-role
// Cette route met à jour le rôle d'un utilisateur et réinitialise le flag upgradeRequested.
// Elle reçoit en body le username et le nouveau rôle (newRole).
//
app.put('/update-role', (req, res) => {
  ...
  // Mise à jour du rôle et réinitialisation du flag
