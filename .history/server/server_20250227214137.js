...
app.post('/api/user/:userId/update-progression', async (req, res) => {
  try {
    ...
    // **1️⃣ Charger les données de connectDatas.json**
    ...
    // **2️⃣ Charger les données de modules.json**
    const modulesData = JSON.parse(await fs.readFile(MODULES_FILE_PATH, 'utf-8'));

    // **3️⃣ Associer les cours validés aux modules**
    let modulesProgress = [];
    ...
    // **4️⃣ Vérifier si l'utilisateur existe déjà dans MongoDB**
    let userProgress = await UserProgress.findOne({ userId });

    if (!userProgress) {
      // **Créer un nouvel enregistrement si l'utilisateur n'existe pas encore**
...
app.post('/api/user/:userId/add-module', async (req, res) => {
  try {
    ...
    if (!moduleId || !moduleName || totalCourses == null) {
      return res.status(400).json({ message: 'Module ID, nom et nombre total de cours sont requis.' });
    }

    let userProgress = await UserProgress.findOne({ userId });

    if (!userProgress) {
      return res.status(404).json({ message: 'Utilisateur non trouvé dans la base MongoDB.' });
    }
...
app.post('/api/user/:userId/validate-course', async (req, res) => {
  try {
    ...
    if (!moduleId || !courseId) {
      return res.status(400).json({ message: 'Module ID et Course ID sont requis.' });
    }
