...
// Route pour récupérer la progression d'un utilisateur depuis MongoDB
...
app.post('/api/users/:userId/update-progression', async (req, res) => {
  try {
    const { userId } = req.params; // Ici userId représente le username, par exemple "1"
    ...
    const modulesProgress = modules.map(module => {
      // On considère que user.validatedCourses contient la liste des cours validés pour cet utilisateur
    ...
    // Rechercher ou créer l'entrée de progression en se basant sur le username
