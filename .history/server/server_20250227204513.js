...
const MODULES_FILE_PATH = './json/modules.json';
...
app.post('/api/user/:userId/update-progression', async (req, res) => {
  try {
    ...
    const modulesProgress = modulesData.map(module => {
      // Liste des identifiants de cours du module
      const moduleCourseIds = module.courses.map(course => course.id);
      // Récupérer les cours validés par l'utilisateur appartenant à ce module
    ...
    }).filter(module => module.totalCourses > 0); // Ne conserver que les modules ayant des cours

    // Lire les données actuelles de userProgress.json
    let progressData;
    try {
      progressData = JSON.parse(await fs.readFile(PROGRESS_FILE, 'utf-8'));
    } catch (error) {
      progressData = [];
    }
    ...
    // Écrire les nouvelles données dans userProgress.json
