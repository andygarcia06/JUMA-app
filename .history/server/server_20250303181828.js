...
app.use((req, res, next) => {
  // Autorise l'origine http://localhost:3000 (ou * pour tout autoriser)
...
const userFilePath = path.join(__dirname, 'json', 'connectDatas.json'); 
// Le fichier où tu stockes tes utilisateurs (celui que tu as montré).
...
// Fonction utilitaire pour écrire dans le JSON
function writeUsersToFile(users) {
  try {
    fs.writeFileSync(userFilePath, JSON.stringify(users, null, 2), 'utf8');
  } catch (err) {
    console.error('Erreur lors de l\'écriture dans users.json :', err);
  }
}

// Route pour mettre à jour les achievements d’un utilisateur
app.post('/api/users/:userId/achievements', (req, res) => {
  const { userId } = req.params;
  // Par exemple, le body contiendra un objet { creationLevel: "Guide", positiveReactionsLevel: "Acclamé(e)" }
  const newAchievements = req.body;

  const users = readUsersFromFile();

  // Selon ta structure, il faut trouver l'utilisateur par 'userId' ou par 'username'.
  // Ici, on suppose que dans le JSON, certains utilisateurs ont un champ "userId",
  // d'autres seulement "username". Il faut donc adapter la logique.
  // Exemple : on cherche d’abord un user qui aurait "userId" === userId :
  let user = users.find(u => u.userId === userId);

  // Si pas trouvé, on tente de trouver par "username" (au cas où userId correspond au username)
  if (!user) {
  ...
  // Si l’utilisateur n’a pas encore d’achievements, on lui en crée un
  if (!user.achievements) {
    user.achievements = {};
  }

  // On fusionne les nouveaux achievements dans l’existant
  // Par exemple : user.achievements.creationLevel = "Guide"
  for (const [key, value] of Object.entries(newAchievements)) {
    user.achievements[key] = value;
  }

  // On écrit la mise à jour dans le fichier JSON
