...
app.post('/api/company/:companyId/members', async (req, res) => {
  try {
    const { companyId } = req.params;
    const { members } = req.body; // On attend ici un tableau de membres, chacun avec par exemple userId et email

    if (!Array.isArray(members)) {
      return res.status(400).json({ message: "Le champ 'members' doit être un tableau." });
    }

    // On recherche l'entreprise dans la collection Company (ou ProjectManagement, selon votre logique)
    ...
    // Fusionner les nouveaux membres en évitant les doublons (ici, on utilise l’identifiant unique _id ou userId)
    members.forEach(member => {
      if (!company.members.some(m => String(m.userId).trim() === String(member.userId).trim())) {
        company.members.push(member);
      }
    });

    await company.save();
    res.status(200).json({ message: 'Membres ajoutés avec succès.', company });
  } catch (error) {
    console.error("[SERVER] Erreur lors de l'ajout des membres :", error);
    res.status(500).json({ message: "Erreur serveur lors de l'ajout des membres." });
  }
});
