...
// Route pour comparer le détail du ticket avec les modules
...
const moduleTicketsPath = path.join(__dirname, 'moduleTickets.json');
  ...
  const moduleTickets = JSON.parse(fs.readFileSync(moduleTicketsPath, 'utf8'));
  ...
  // Fonction pour comparer le détail avec une entrée (exemple simple de correspondance)
  const compareDetail = (detail, entry) => {
    return detail.includes(entry.someField); // Remplacez 'someField' par le champ pertinent
  };

  // Comparer avec les modules
  const matchingModules = modules.filter(module => compareDetail(detail, module));

  // Comparer avec les moduleTickets
  const matchingModuleTickets = moduleTickets.filter(mt => compareDetail(detail, mt));
