...
// Connexion à MongoDB
mongoose.connect('mongodb://127.0.0.1:27017/jumaDB', { // "jumaDB" est la DB qui sera créée automatiquement
...
app.post('/validated-course', async (req, res) => {
  try {
    ...
    let jsonData = await fs.readFile(PROGRESS_FILE, { encoding: 'utf-8' });
...
// PROGRESION MODULE
const PROGRESS_FILE = './json/userProgress.json'; // 📌 Fichier JSON pour la progression des utilisateurs


// 📌 Route pour mettre à jour la progression d'un utilisateur
app.post('/api/user/:userId/update-progression', async (req, res) => {
  try {
    const { userId } = req.params;

    // Lire les données utilisateurs et modules
    const connectData = JSON.parse(await fs.readFile(CONNECT_DATA_FILE, { encoding: 'utf-8' }));
    const modulesData = JSON.parse(await fs.readFile(MODULES_FILE_PATH, { encoding: 'utf-8' }));

    const user = connectData.find(u => u.username === userId || u.userId === userId);
    if (!user || !user.courses) {
      return res.status(404).json({ message: "Aucun cours validé trouvé pour cet utilisateur." });
    }

    // Associer les cours validés aux modules
    let modulesProgress = [];
    modulesData.forEach(module => {
      const moduleCourses = module.courses.map(course => course.id);
      const validatedCourses = user.courses.filter(courseId => moduleCourses.includes(courseId));
    ...
    // Lire et mettre à jour userProgress.json
    let progressData;
    try {
      progressData = JSON.parse(await fs.readFile(PROGRESS_FILE, { encoding: 'utf-8' }));
    } catch (error) {
      progressData = []; // Si le fichier est vide ou inexistant
    }
...
app.get('/api/user/:userId/progression', async (req, res) => {
  try {
    ...
    try {
      ...
      return res.status(404).json({ message: "Aucune donnée de progression trouvée." });
    }

    const userProgress = progressData.find(u => u.userId === userId);
    if (!userProgress) {
      return res.status(404).json({ message: "Utilisateur non trouvé dans la base de progression." });
    }

    let totalCourses = 0;
    let validatedCourses = 0;

    userProgress.modules.forEach(module => {
      totalCourses += module.totalCourses;
      validatedCourses += module.validatedCourses.length;
    });

    const progressPercentage = totalCourses > 0 ? (validatedCourses / totalCourses) * 100 : 0;

    res.status(200).json({
      userId,
      username: userProgress.username,
      company: userProgress.company,
      progress: `${progressPercentage.toFixed(2)}%`,
      validatedCoursesCount: validatedCourses,
...
/** 
* 🔹 Ajoute un module à la progression d'un utilisateur
*/
app.post('/api/user/:userId/add-module', async (req, res) => {
 try {
   const { userId } = req.params;
   const { moduleId, moduleName, totalCourses } = req.body;

   let progressData = JSON.parse(await fs.readFile(PROGRESS_FILE, 'utf-8'));
   let userProgress = progressData.find(u => u.userId === userId);

   if (!userProgress) {
     return res.status(404).json({ message: 'Utilisateur non trouvé dans la base JSON.' });
   }

   if (userProgress.modules.some(m => m.moduleId === moduleId)) {
     return res.status(400).json({ message: 'Module déjà ajouté pour cet utilisateur.' });
   }

   userProgress.modules.push({ moduleId, moduleName, totalCourses, validatedCourses: [] });
   await fs.writeFile(PROGRESS_FILE, JSON.stringify(progressData, null, 2));

   res.status(200).json({ message: 'Module ajouté avec succès.', userProgress });
...
/** 
* 🔹 Marque un cours comme validé pour un utilisateur
*/
app.post('/api/user/:userId/validate-course', async (req, res) => {
 try {
   const { userId } = req.params;
   ...
   let module = userProgress.modules.find(m => m.moduleId === moduleId);

   if (!module) {
     return res.status(404).json({ message: 'Module non trouvé pour cet utilisateur.' });
   }

   if (!module.validatedCourses.includes(courseId)) {
     module.validatedCourses.push(courseId);
     await fs.writeFile(PROGRESS_FILE, JSON.stringify(progressData, null, 2));
   }

   res.status(200).json({ message: 'Cours validé avec succès.', userProgress });
