...
app.post('/upgrade-request', (req, res) => {
  ...
  // On ne traite que les enregistrements qui possèdent la propriété "username"
  const userIndex = users.findIndex(u => u.username && u.username === username);
  if (userIndex === -1) {
    console.log("[SERVER] User not found for username:", username);
    return res.status(404).json({ success: false, error: 'Utilisateur non trouvé' });
  }
 ...
 * Renvoie la liste des utilisateurs (ayant une propriété "username") dont "upgradeRequested" est true
 * et dont le rôle est "utilisateur".
 */
 ...
 * et en réinitialisant le flag "upgradeRequested".
 */
