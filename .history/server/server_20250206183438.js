...
app.get('/api/pending-companies', (req, res) => {
  let pendingCompanies = loadCompaniesDatabase();

  // 🔥 Correction : Assurer que chaque entreprise a un champ `members`
  pendingCompanies = pendingCompanies.map(company => ({
    ...company,
    members: Array.isArray(company.members) ? company.members : [], // Si `members` est undefined, on met []
  }));

  console.log("✅ Données envoyées par l'API (corrigé) :", JSON.stringify(pendingCompanies, null, 2));
