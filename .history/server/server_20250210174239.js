...
app.get("/api/project-meteo/:ticketId", (req, res) => {
  ...
  const updateTicketsDB = (ticketId, meteo) => {
    ...
    console.log("📌 Tickets avant mise à jour:", JSON.stringify(tickets, null, 2));

    const ticketIndex = tickets.findIndex(t => t.id === ticketId);
    console.log("📌 Index du ticket trouvé:", ticketIndex);

    if (ticketIndex === -1) {
        console.error("❌ Ticket non trouvé, impossible de mettre à jour la météo.");
        return;
    }

    // 🔹 Mise à jour de la météo du ticket
    tickets[ticketIndex].meteo = meteo;
    console.log("✅ Nouvelle valeur du ticket mis à jour:", JSON.stringify(tickets[ticketIndex], null, 2));
    ...
    // 🔹 Vérifier si l'écriture a bien fonctionné
    const ticketsAfterWrite = readJSONFile(DB_TICKETS_PATH);
    console.log("📌 Tickets après écriture:", JSON.stringify(ticketsAfterWrite, null, 2));
};


  // 📌 ✅ Mise à jour de `tickets.json` avec la météo
  console.log(`📌 Appel de updateTicketsDB avec ticketId: ${ticketId} et meteo: ${meteo}`);
