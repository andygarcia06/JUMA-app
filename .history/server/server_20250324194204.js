...
app.get('/api/company/:companyName/members', async (req, res) => {
  try {
    ...
    // On peut ajouter un trim et une conversion en minuscule si besoin
const company = await ProjectCompany.findOne({ companyName: { $regex: new RegExp(`^${companyName.trim()}$`, "i") } });
    if (!company) {
