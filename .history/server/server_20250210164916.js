...
// 📂 Chemin du fichier JSON (stocke les tickets d'enrichissement)
...
app.route("/api/enrich-db")
  ...
  // 📌 POST : Ajouter un nouveau ticket
  .post((req, res) => {
...
// 📌 Route pour récupérer la météo du projet
app.get("/api/project-meteo/:ticketId", (req, res) => {
  ...
  const messages = readJSONFile(DB_MESSAGES_PATH)[ticketId] || [];

  // 🔹 Trouver le créateur du ticket
  const ticket = tickets.find(t => t.id === ticketId);
  if (!ticket) {
    return res.status(404).json({ success: false, message: "❌ Ticket non trouvé." });
  }

  const ticketCreatorId = ticket.userId; // Récupération du `userId` du créateur

  // 🔹 Filtrer uniquement les messages du créateur du ticket
  const creatorMessages = messages.filter(msg => msg.userId === ticketCreatorId);
  ...
  // 🔹 Comparer chaque message du créateur avec les entrées stockées
