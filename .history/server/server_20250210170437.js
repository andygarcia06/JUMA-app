...
app.get("/api/project-meteo/:ticketId", (req, res) => {
  ...
  const messages = readJSONFile(DB_MESSAGES_PATH);
  ...
  // 🔹 Filtrer les messages de `messages.json` qui :
  //   - Sont liés à ce `ticketId`
  //   - Ont été envoyés par le `userId` du créateur du ticket
  const creatorMessages = messages[ticketId]?.filter(msg => msg.userId === ticketCreatorId) || [];
