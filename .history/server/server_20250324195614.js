...
db.projectcompanies.find({ companyName: /Sequoaisoft/i })

app.get('/api/company/:companyName/members', async (req, res) => {
  try {
    const { companyName } = req.params;
    console.warn(`[SERVER] Recherche des membres pour la compagnie: "${companyName}"`);

    // Afficher toutes les compagnies présentes dans la collection pour déboguer
    const allCompanies = await ProjectManagement.find({});
    console.warn('[SERVER] Toutes les compagnies dans la DB:', allCompanies.map(c => c.companyName));

    // Rechercher la compagnie de manière insensible à la casse
