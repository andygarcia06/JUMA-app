...
// Route POST pour ajouter une tab à un projetapp.post('/projects/:projectId/tabs', async (req, res) => {
  app.post('/projects/:projectId/tabs', async (req, res) => {
    try {
      ...
      // Récupérer le document DataCompanies (on suppose qu'il n'y a qu'un seul document)
      let dataCompaniesDoc = await DataCompanies.findOne({});
      if (!dataCompaniesDoc) {
        // S'il n'existe pas, on le crée avec un tableau vide de companies
      ...
      // Vérifier si la tab existe déjà
      if (project.tabs.some(t => t.tabId === tabId)) {
...
app.post('/tabs/:tabId/rows', async (req, res) => {
  try {
    ...
    // Chercher la donnée dans DataCompanies
    let dataCompanies = await DataCompanies.findOne({ 'companies.id': companyId });
    if (!dataCompanies) {
      return res.status(404).json({ message: "Company not found" });
    }
    
    // Rechercher la compagnie dans le tableau companies
    let company = dataCompanies.companies.find(c => c.id === companyId);
    if (!company) {
    ...
    let project = program.projects.find(proj => proj.id === projectId);
    if (!project) {
...
app.get('/tabs/:tabId/rows', async (req, res) => {
  try {
    ...
    console.log(`🔍 Recherche des rows pour Tab ID: ${tabId}, Projet ID: ${projectId}, Company ID: ${companyId}, Program ID: ${programId}`);
    ...
    const company = dataCompany.companies.find(c => c.id === companyId);
    ...
    console.log(`✅ Rows trouvées pour la tab:`, tab.rows);
