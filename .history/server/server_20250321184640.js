...
// Mise à jour de la progression de l'utilisateur dans MongoDB (basée sur le userId reçu dans KEM)
app.post('/api/users/:userId/update-progression', async (req, res) => {
  try {
    const { userId } = req.params; // Ici, userId est celui reçu depuis KEM, par exemple "67d09cea89704d5dade1bd97"
    console.log('[SERVER] Requête update-progression pour userId:', userId);

    // Recherche l'utilisateur par son userId (tel que stocké dans la DB utilisateur)
    ...
    // Pour chaque module, utiliser le nombre total de cours du module (module.courses.length)
    // et déterminer quels cours validés par l'utilisateur appartiennent à ce module.
    const modulesProgress = modules.map(module => {
      const totalCourses = module.courses.length; // Nombre total de cours dans ce module
      // Filtrer les cours validés par l'utilisateur (user.courses) appartenant à ce module
    ...
    // Recherche (ou création) du document UserProgress en se basant sur le userId (celui reçu dans KEM)
    let userProgress = await UserProgress.findOne({ userId: user.userId });
    if (!userProgress) {
      userProgress = new UserProgress({
        userId: user.userId, // Par exemple "67d09cea89704d5dade1bd97"
