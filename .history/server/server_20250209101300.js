...
const calculateSimilarity = (text1, text2) => {
  if (!text1 || !text2) {
    ...
    return 0; // Retourner 0% de similarité si un texte est invalide
  }
...
app.get('/api/compare/:ticketId', (req, res) => {
  ...
  try {
    ...
    const ticketDetail = ticket.detail?.trim(); // Supprime les espaces inutiles
    ...
    // Seuil de similarité (en pourcentage) pour considérer deux textes comme similaires
    const similarityThreshold = 70;

    // Comparer avec modules.json
    const matchingModules = modulesData.filter(module => {
      if (!Array.isArray(module.courses)) return false;
    
      return module.courses.some(course => {
        if (!course.content || typeof course.content !== 'string') {
          console.warn("⚠️ Attention: `course.content` est invalide ou manquant.", course);
          return false; // Ignore ce cours
        }
    
        const similarity = calculateSimilarity(ticketDetail, course.content.trim());
    ...
    const matchingModuleTickets = moduleTicketsData.filter(moduleTicket => {
      const moduleTicketContent = moduleTicket.content?.trim();
      if (!moduleTicketContent || typeof moduleTicketContent !== 'string') {
        console.warn("Attention: `moduleTicket.content` est invalide.", { moduleTicket });
        return false;
      }
      const similarity = calculateSimilarity(ticketDetail, moduleTicketContent);
      return similarity >= similarityThreshold;
    });
    ...
    const results = {
      matchingModules,
      matchingModuleTickets
    };

    res.json(results);
  } catch (error) {
    console.error('Erreur lors de la comparaison des modules :', error);
    res.status(500).json({ error: 'Erreur interne du serveur' });
  }
});
