...
app.post('/api/modules/:moduleId/courses/:courseId/reactions', async (req, res) => {
  try {
    const { moduleId, courseId } = req.params;
    const { userId, reactionType } = req.body;
    ...
    course.reactions.push({ userId, reactionType });
    await module.save();

    res.status(201).json({ message: 'Réaction ajoutée avec succès' });
  } catch (error) {
    console.error("[SERVER] Erreur lors de l'ajout de la réaction :", error);
    res.status(500).json({ message: "Erreur lors de l'ajout de la réaction" });
  }
});
