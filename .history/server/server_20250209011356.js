...
app.get('/api/compare/:ticketId', (req, res) => {
  ...
  try {
    ...
    // Fonction de comparaison (à adapter selon vos critères)
    const compareText = (text1, text2) => {
      // Exemple simple : vérifier si text2 est inclus dans text1
      return text1.includes(text2);
    };

    // Comparer avec modules.json
    const matchingModules = modulesData.filter(module => compareText(ticketDetail, module.content));
