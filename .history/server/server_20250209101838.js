...
app.get('/api/compare/:ticketId', (req, res) => {
  ...
  // Nettoyage du texte (supprime les balises HTML et espaces inutiles)
  ...
  // Calcul de similarité avec l'algorithme de Levenshtein
  const calculateSimilarity = (text1, text2) => {
    if (!text1 || !text2) {
      console.error("❌ Erreur: l'un des textes est undefined ou vide.", { text1, text2 });
      return 0; // Retourner 0% si l'un des textes est invalide
    }
  ...
  try {
    ...
    const matchingModuleTickets = moduleTicketsData.filter(moduleTicket => {
      if (!moduleTicket.content || typeof moduleTicket.content !== 'string' || moduleTicket.content.trim() === "") {
        console.warn("⚠️ `moduleTicket.content` est invalide.", moduleTicket);
        return false;
      }
      ...
      console.log(`🔍 Similarité [ModuleTicket]: ${similarity}%`);
