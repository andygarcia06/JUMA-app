...
const moduleTicketsPath = path.join(__dirname, 'moduleTickets.json');
...
app.get('/api/compare/:ticketId', (req, res) => {
  ...
  try {
    ...
    // Fonction de comparaison simple
    const compareText = (text1, text2) => {
      return text1.toLowerCase().includes(text2.toLowerCase());
    };

    // Comparer avec modules.json (y compris les contenus des cours)
    const matchingModules = modulesData.filter(module => {
      // Comparer avec le contenu du module
      if (compareText(ticketDetail, module.content)) {
        return true;
      }
      // Si le module a des cours, comparer avec le contenu de chaque cours
      if (module.courses && Array.isArray(module.courses)) {
        return module.courses.some(course => compareText(ticketDetail, course.content));
      }
      return false;
    });

    // Comparer avec moduleTickets.json
    const matchingModuleTickets = moduleTicketsData.filter(moduleTicket => compareText(ticketDetail, moduleTicket.content));
