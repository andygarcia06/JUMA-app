...
const http = require('http'); // Ajoute cette ligne pour utiliser un serveur HTTP
...
// Utilisation d'un chemin unique pour lire et écrire dans le JSON
const filePath = path.join(__dirname, './json/connectDatas.json');
console.log("[SERVER] filePath:", filePath);

//
// POST /upgrade-request
// Reçoit { username } dans le body et met à jour l'utilisateur dont la propriété "username" correspond.
// Seuls les objets comportant une propriété "username" seront considérés (pour ne pas modifier les autres enregistrements).
//
app.post('/upgrade-request', (req, res) => {
  const { username } = req.body;
  ...
  // On recherche uniquement dans les enregistrements qui possèdent la propriété username
  const userIndex = users.findIndex(u => u.username && u.username === username);
  if (userIndex === -1) {
...
// Retourne la liste des demandes d'upgrade : uniquement les utilisateurs possédant
// une propriété "username", ayant "upgradeRequested" à true et dont le rôle est "utilisateur".
//
app.get('/upgrade-requests', (req, res) => {
  ...
  // On filtre uniquement les enregistrements utilisateur avec upgradeRequested === true
  const upgradeRequests = users.filter(u => u.username && u.upgradeRequested === true && u.role === 'utilisateur');
...
//
// PUT /update-role
// Met à jour le rôle d'un utilisateur (identifié par username) et réinitialise le flag upgradeRequested.
// Reçoit { username, newRole } dans le body.
//
app.put('/update-role', (req, res) => {
  const { username, newRole } = req.body;
  console.log("[SERVER] Received update-role for username:", username, "with newRole:", newRole);
  
  let users = [];
  try {
    const data = fs.readFileSync(filePath, 'utf8');
    users = JSON.parse(data);
    console.log("[SERVER] Current users before update:", users);
  } catch (err) {
    console.error("[SERVER] Error reading JSON file:", err);
    return res.status(500).json({ success: false, error: 'Erreur serveur - lecture JSON' });
  }
  ...
  // Réinitialiser le flag upgradeRequested une fois le rôle mis à jour
  users[userIndex].upgradeRequested = false;
  console.log("[SERVER] User updated:", users[userIndex]);
...
app.get('/api/suggestions/suggestModules', (req, res) => {
  ...
  // Obtenir les synonymes du terme de recherche
  const relatedTerms = synonymsData[query] || [];
  relatedTerms.push(query);  // Ajoute le terme de recherche principal aux synonymes

  // Filtrer les tickets ayant un des termes liés dans leurs détails
  const matchingTickets = ticketsData.filter(ticket => {
      const fieldsToSearch = `${ticket.detail} ${ticket.comments} ${ticket.rule} ${ticket.title}`.toLowerCase();
      return relatedTerms.some(term => fieldsToSearch.includes(term));
  });

  if (matchingTickets.length === 0) {
      return res.status(404).json({ error: 'Aucun ticket correspondant trouvé.' });
  }

  // Calcul des statistiques d'apparition des modules dans les tickets filtrés
  const moduleFrequency = matchingTickets.reduce((acc, ticket) => {
      const moduleId = ticket.selectedModule;
      if (moduleId) {
  ...
  // Calculer le pourcentage pour chaque module basé sur le nombre total de tickets correspondants
  const moduleSuggestions = Object.entries(moduleFrequency).map(([moduleId, count]) => {
      const percentage = ((count / matchingTickets.length) * 100).toFixed(2);
      const moduleInfo = modulesData.find(module => module.id === moduleId) || {};
      return { moduleId, moduleTitle: moduleInfo.title || 'Module inconnu', percentage };
  });

  // Trier les modules suggérés par pourcentage de correspondance (descendant)
  moduleSuggestions.sort((a, b) => b.percentage - a.percentage);

  res.json(moduleSuggestions);
});
