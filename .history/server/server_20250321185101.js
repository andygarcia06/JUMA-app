...
// On reçoit l'_id de l'utilisateur dans l'URL, on récupère le username associé, puis on recherche la progression avec ce username
app.post('/api/users/:userId/update-progression', async (req, res) => {
  try {
    const { userId } = req.params; // Ici, userId est l'_id (ex. "67d09cea89704d5dade1bd97")
    console.log('[SERVER] Requête update-progression pour userId:', userId);

    // Recherche de l'utilisateur par son _id pour obtenir le username
    const user = await User.findById(userId);
    if (!user) {
      return res.status(404).json({ message: 'Utilisateur non trouvé.' });
    }
    
    // Récupère le username associé (par exemple "ansy" ou "1")
    const username = user.username;
    console.log('[SERVER] Username trouvé:', username);

    // Récupérer tous les modules depuis la collection Module
    const modules = await Module.find({});
    
    // Pour chaque module, on utilise le nombre total de cours défini dans le module (module.courses.length)
    // et on filtre les cours validés de l'utilisateur (stockés dans user.courses)
    ...
    // Recherche (ou création) du document de progression dans UserProgress
    // Ici, on utilise le username comme identifiant (stocké dans le champ userId de UserProgress)
    let userProgress = await UserProgress.findOne({ userId: username });
    if (!userProgress) {
      userProgress = new UserProgress({
        userId: username, // Par exemple "ansy" ou "1"
