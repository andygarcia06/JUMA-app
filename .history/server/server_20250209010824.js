...
// Route pour comparer le ticketDetail avec les modules
router.get('/api/suggestions/compareModules', (req, res) => {
  ...
  const moduleTickets = loadJSON(moduleTicketsPath);

  // Fonction de comparaison (par exemple, recherche de correspondances)
  const findMatchingModules = (text, data) => {
    return data.filter((item) => item.detail && item.detail.includes(text));
  };

  // Comparer le ticketDetail avec les deux ensembles de données
  const matchingModules = findMatchingModules(query, modules);
  const matchingModuleTickets = findMatchingModules(query, moduleTickets);
