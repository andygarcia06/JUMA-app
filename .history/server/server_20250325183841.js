...
// Profil and updateProfile
// Route POST pour mettre à jour le profil utilisateur
...
const storage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    // Génère un nom de fichier unique en préfixant par la date
...
app.post('/api/update-profile', upload.fields([
  ...
  try {
    // Récupérer les champs texte et d'adresse
    const { username, bio, email, phoneNumber, street, city, state, postalCode, country } = req.body;

    // Préparer l'objet de mise à jour avec les nouveaux champs
    const updateData = {
      bio,
      email,
      phoneNumber,
    ...
    // Si des fichiers ont été uploadés, ajouter leurs chemins
    if (req.files['profilePic']) {
      updateData.profilePicUrl = `/uploads/${req.files['profilePic'][0].filename}`;
    }
    if (req.files['backgroundPic']) {
      updateData.backgroundPicUrl = `/uploads/${req.files['backgroundPic'][0].filename}`;
    }

    // Mettre à jour l'utilisateur dans la DB (exemple avec Mongoose)
...
app.put('/api/update-profile', upload.fields([
  { name: 'profilePic', maxCount: 1 },
  { name: 'backgroundPic', maxCount: 1 }
]), async (req, res) => {
  try {
    console.log("----- REQUÊTE DE MISE À JOUR DU PROFIL -----");
    console.log("req.body:", req.body);
    console.log("req.files:", req.files);

    const { username, userId, bio, email, phoneNumber, street, city, state, postalCode, country } = req.body;
    ...
    if (userId && userId !== 'undefined') {
      query = { _id: new mongoose.Types.ObjectId(userId) }; // Correction ici
...
app.get('/api/users/:identifier', async (req, res) => {
  ...
  try {
    let user = null;
    // Si l'identifiant est un ObjectId valide, on recherche par _id
    if (mongoose.Types.ObjectId.isValid(identifier)) {
      user = await User.findById(identifier);
    }
    // Sinon, ou si aucune correspondance n'est trouvée, on recherche par username
    if (!user) {
...
// Route GET pour récupérer le profil complet de l'utilisateur
app.get('/api/users/:userId', async (req, res) => {
  try {
    const { userId } = req.params;
    // Ici, nous utilisons le champ "username" pour rechercher l'utilisateur
    ...
    console.error("[SERVER] Erreur lors de la récupération du profil :", error);
...
app.post('/api/courses', async (req, res) => {
  ...
  // On récupère moduleId au lieu de moduleName
  const { title, moduleId, description, content, createdAt, creator } = req.body;
  if (!title || !moduleId || !description || !content || !createdAt || !creator) {
    return res.status(400).json({ message: 'Veuillez fournir toutes les informations requises.' });
  }

  try {
    // On cherche le module par son id
    const module = await Module.findOne({ id: moduleId });
    if (!module) {
      return res.status(404).json({ message: 'Module non trouvé' });
    }

    // Vérifier qu'aucun cours avec le même titre n'existe déjà dans ce module
    ...
    const newCourse = {
      id: new mongoose.Types.ObjectId().toString(),
      title,
      // Pour information, on peut aussi stocker le titre du module si nécessaire
      moduleId: module.id,
      description,
      content,
      createdAt: new Date(),
...
// Route pour récupérer la progression d'un utilisateur (basée sur son username)
app.get('/api/users/:userId/progression', async (req, res) => {
  const { userId } = req.params; // Ici, userId représente le username
  try {
    // Recherche du document de progression par le username
    const userProgress = await UserProgress.findOne({ userId });
    if (!userProgress) {
      return res.status(404).json({ message: 'Progression non trouvée pour cet utilisateur.' });
    }

    // Calcul de la progression globale (facultatif)
    let totalCourses = 0;
...
// Mise à jour de la progression de l'utilisateur dans MongoDB (basée sur le pseudo)
app.post('/api/users/:userId/update-progression', async (req, res) => {
  try {
    const { userId } = req.params; // Ici, userId représente le pseudo, par exemple "1" ou "ansy"
    console.log('[SERVER] Requête update-progression pour pseudo:', userId);

    // Recherche de l'utilisateur par son username (puisque userId représente le pseudo)
    const user = await User.findOne({ username: userId });
    if (!user) {
      return res.status(404).json({ message: 'Utilisateur non trouvé.' });
    }
    console.log('[SERVER] Utilisateur trouvé:', user);

    // Recherche dans UserValidateCourse (si tu l'utilises) en utilisant le champ userId
    const userValidateEntry = await UserValidateCourse.findOne({ userId: user.userId });
    console.log('[SERVER] UserValidateEntry trouvé:', userValidateEntry);

    // Récupérer tous les modules depuis la collection Module
    const modules = await Module.find({});

    // Pour chaque module, calculer la progression en utilisant le nombre total de cours du module
    // et en filtrant les cours validés de l'utilisateur (user.courses)
    const modulesProgress = modules.map(module => {
      const totalCourses = module.courses.length;
      const validatedCourses = user.courses
        ? user.courses.filter(courseId => module.courses.some(course => course.id === courseId))
        : [];
    ...
    // Recherche (ou création) du document UserProgress en se basant sur le pseudo
    let userProgress = await UserProgress.findOne({ userId: user.username });
    if (!userProgress) {
      userProgress = new UserProgress({
        userId: user.username,  // Ici on stocke le pseudo, par exemple "1" ou "ansy"
    ...
    console.log('[SERVER] Progression mise à jour:', userProgress);
...
app.get('/api/modules/:moduleId/courses/:courseId/reactions', async (req, res) => {
  try {
    const { moduleId, courseId } = req.params;
    // On interroge la base MongoDB via le modèle Module
    const module = await Module.findOne({ id: moduleId });
    if (!module) {
      return res.status(404).json({ message: 'Module non trouvé' });
    }
...
app.get('/api/user/:userId/most-viewed-modules', async (req, res) => {
  try {
    // Ici, le paramètre userId représente le pseudo (ex. "1" ou "ansy")
    const { userId } = req.params;
    console.log('[BACK] Requête pour les modules les plus consultés pour pseudo:', userId);

    // Rechercher dans ProjectManagement une société dont la liste des membres contient ce pseudo
    // OU dont le champ "userId" du document correspond au pseudo (si c'est utilisé pour l'entreprise)
    ...
    if (!company) {
      console.log('[BACK] Aucun company trouvé pour le pseudo:', userId);
      return res.status(404).json({ message: 'Société non trouvée pour cet utilisateur.' });
    }
    console.log('[BACK] Société trouvée:', company.companyName);

    // Récupérer la liste des membres de la société (les identifiants utilisés dans ProjectManagement)
    const companyMemberIds = company.members.map(member => member.userId);
    // On peut aussi ajouter le userId principal de l'entreprise s'il y a lieu
    if (company.userId) {
      companyMemberIds.push(company.userId);
    }
    console.log('[BACK] Membres utilisés pour filtrer les modules:', companyMemberIds);

    // Récupérer les modules dont le créateur (creator.userId) est dans la liste des membres
    const modules = await Module.find({ "creator.userId": { $in: companyMemberIds } });
    console.log('[BACK] Nombre de modules récupérés:', modules.length);

    // Pour chaque module, calculer le nombre total de réactions (en parcourant les cours)
    const modulesWithReactions = modules.map(module => {
      let totalReactions = 0;
      if (Array.isArray(module.courses)) {
        module.courses.forEach(course => {
          if (Array.isArray(course.reactions)) {
            totalReactions += course.reactions.length;
          }
        });
      }
      return {
        id: module.id,
        title: module.title,
        courses: module.courses, // On renvoie les cours pour permettre l'affichage dans la modal
        reactions: totalReactions
      };
    });
    console.log('[BACK] Modules avec réactions (avant tri):', modulesWithReactions);

    // Trier par ordre décroissant de réactions
    modulesWithReactions.sort((a, b) => b.reactions - a.reactions);
    console.log('[BACK] Modules triés par popularité:', modulesWithReactions);

    res.status(200).json(modulesWithReactions);
  } catch (error) {
    console.error('[BACK] Erreur lors de la récupération des modules les plus consultés :', error);
    res.status(500).json({ message: 'Erreur serveur lors de la récupération des modules les plus consultés.' });
  }
});
