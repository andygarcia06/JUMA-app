...
const WebSocket = require('ws');
 // Ajoute cette ligne pour utiliser un serveur HTTP
...
app.post('/api/tickets/:ticketId/validate', (req, res) => {
    const { ticketId } = req.params;
    const { userId, action } = req.body; // action : "validate" ou "reject"

    fs.readFile(dbFilePath, 'utf8', (err, data) => {
        if (err) return res.status(500).json({ message: "Erreur lecture DB", error: err });
        ...
        // Vérifier si l'utilisateur est bien le créateur du ticket (user.userId)
        if (!ticket.user || ticket.user.userId !== userId) {
            return res.status(403).json({ message: "Seul le créateur peut valider ce ticket" });
        }

        // Mettre à jour le statut en fonction de l'action
        if (action === "validate") {
            ticket.pendingValidationTicket = "validated";
        } else if (action === "reject") {
            ticket.pendingValidationTicket = "waiting";
        } else {
            return res.status(400).json({ message: "Action non valide" });
        }

        // Sauvegarde dans la DB
        fs.writeFile(dbFilePath, JSON.stringify(tickets, null, 2), (writeErr) => {
            if (writeErr) return res.status(500).json({ message: "Erreur écriture DB", error: writeErr });
