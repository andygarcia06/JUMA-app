...
app.get('/api/pending-companies', async (req, res) => {
  try {
    ...
    // Ici on recherche par pendingValidation: true, si c'est votre critère
    ...
    let ProjectManagement = await ProjectCompany.find();

    // Fusionner les `members` depuis `ProjectCompany` dans `Company`
    pendingCompanies = pendingCompanies.map(company => {
      const projectCompany = ProjectManagement.find(proj => proj.id === company.id);
      return {
        ...company.toObject(),
