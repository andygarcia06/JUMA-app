...
app.get('/dashboard-project-assigned/:userId', (req, res) => {
  const { userId } = req.params;
  console.log(`[SERVER] => Reçu GET /dashboard-project-assigned/${userId}`);
  ...
  try {
    ...
    console.log('[SERVER] => Contenu de projectmanagement.json chargé avec succès.');

    const assignedProjects = [];

    // Parcourir chaque compagnie
    projectManagementData.forEach((company, idxCompany) => {
      console.log(`\n[SERVER] [COMPANY ${idxCompany}] => ${company.companyName} (ID: ${company.id})`);

      // 1) Vérifier si l'utilisateur est membre de la compagnie
      const isCompanyMember = company.members?.some(m => m.userId === userId);
      console.log(`[SERVER]   - userId="${userId}" est-il dans company.members? =>`, isCompanyMember);

      if (!isCompanyMember) {
        // Si l'utilisateur n'est pas member de cette compagnie, on saute
        console.log(`[SERVER]   - Ignoré, car userId="${userId}" n'est pas member de ${company.companyName}`);
        return;
      }

      // 2) Vérifier s'il existe des programmes
      if (!company.programs || !Array.isArray(company.programs)) {
        console.log(`[SERVER]   - Pas de "programs" (ou mauvais format) pour ${company.companyName}`);
        return;
      }

      // Parcourir chaque programme
      company.programs.forEach((program, idxProgram) => {
        console.log(`[SERVER]     [PROGRAM ${idxProgram}] => ${program.programName} (ID: ${program.programId})`);
        ...
        console.log(`[SERVER]       userId="${userId}" est-il dans program.participants? =>`, isProgramParticipant);

        if (!isProgramParticipant) {
          console.log(`[SERVER]       - Ignoré, userId="${userId}" n'est pas participant du programme "${program.programName}".`);
          return;
        }

        // 3) Parcourir chaque projet du programme
        if (!program.projects || !Array.isArray(program.projects)) {
          console.log(`[SERVER]       - Pas de "projects" (ou mauvais format) dans "${program.programName}".`);
          return;
        }

        program.projects.forEach((proj, idxProject) => {
          console.log(`[SERVER]       [PROJECT ${idxProject}] => "${proj.projectName || proj.id}" (ID: ${proj.id})`);

          // Vérifier si le userId se trouve dans proj.participants (tableau de userIds)
          if (Array.isArray(proj.participants) && proj.participants.includes(userId)) {
            console.log(`[SERVER]         [OK] userId="${userId}" est participant dans le projet "${proj.projectName}".`);
            ...
            console.log(`[SERVER]         [NO] userId="${userId}" n'est pas dans proj.participants.`);
          }
        });
      });
    });

    console.log('\n[SERVER] => assignedProjects final =', assignedProjects);
    return res.json({ assignedProjects });
  } catch (error) {
    console.error('[SERVER] => Erreur /dashboard-project-assigned :', error);
