...
// - Pour la collection Users, on recherche par pseudo (username)
// - Pour UserValidateCourse, on recherche par le champ userId (long id)
app.post('/api/users/:userId/update-progression', async (req, res) => {
  try {
    const { userId } = req.params; // Ici, userId représente le pseudo (ex. "1" ou "ansy")
    console.log('[SERVER] Requête update-progression pour pseudo:', userId);

    // Recherche de l'utilisateur par son pseudo (username)
    const user = await User.findOne({ username: userId });
    if (!user) {
      return res.status(404).json({ message: 'Utilisateur non trouvé.' });
    }
    ...
    // Recherche dans UserValidateCourse en utilisant le champ userId (le long id)
    ...
    // Pour chaque module, on utilise module.courses.length pour le nombre total de cours
    // et on filtre les cours validés de l'utilisateur (user.courses)
    ...
    // Recherche (ou création) du document UserProgress en se basant sur le pseudo (stocké dans user.username)
    let userProgress = await UserProgress.findOne({ userId: user.username });
    if (!userProgress) {
      userProgress = new UserProgress({
        userId: user.username, // On stocke ici le pseudo
