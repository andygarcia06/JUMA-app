...
// Mise à jour de la progression de l'utilisateur dans MongoDB (basée sur le username)
app.post('/api/users/:userId/update-progression', async (req, res) => {
  try {
    const { userId } = req.params; // Ici, userId correspond au username, par exemple "1" ou "ansy"
    console.log('[SERVER] Requête update-progression pour username:', userId);

    // Recherche l'utilisateur par son username (et non par _id)
    const user = await User.findOne({ username: userId });
    if (!user) {
      return res.status(404).json({ message: 'Utilisateur non trouvé.' });
    }
    ...
    const modulesProgress = modules.map(module => {
      // Ici, on utilise user.courses (si ce champ contient les identifiants des cours validés)
    ...
    // Recherche (ou création) de l'entrée de progression dans la collection UserProgress en se basant sur le username
    let userProgress = await UserProgress.findOne({ userId: user.username });
    if (!userProgress) {
      userProgress = new UserProgress({ userId: user.username, modules: modulesProgress });
    } else {
