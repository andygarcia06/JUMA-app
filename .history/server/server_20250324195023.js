...
app.get('/api/company/:companyName/members', async (req, res) => {
  try {
    ...
    // Afficher toutes les compagnies présentes dans la base pour le debug
    const companies = await ProjectCompany.find({});
    console.log('[SERVER] Liste des compagnies dans la DB:', companies.map(c => c.companyName));
