...
app.get('/dashboard-project-assigned/:userId', (req, res) => {
  const { userId } = req.params;
  console.log(`[SERVER] Reçu la requête GET /dashboard-project-assigned/${userId}`);
  ...
  try {
    ...
    projectManagementData.forEach(company => {
      ...
      console.log(`   -> userId="${userId}" est-il dans company.members?`, isCompanyMember);

      if (!isCompanyMember) {
        // Si l'utilisateur n'est même pas "member" de cette compagnie, on ignore
        return;
      }

      // 2) Vérifier l'existence de programmes
      if (!company.programs || !Array.isArray(company.programs)) {
        console.log('   -> Pas de "programs" dans cette compagnie ou format invalide.');
        return;
      }

      company.programs.forEach(program => {
        console.log(`   -> Programme: "${program.programName}" (ID: ${program.programId})`);
        ...
        console.log(`      userId="${userId}" est-il dans program.participants?`, isProgramParticipant);

        if (!isProgramParticipant) {
          // L'utilisateur n'est pas participant dans ce programme
          return;
        }

        // 3) Parcourir chaque projet du programme
        if (!program.projects || !Array.isArray(program.projects)) {
          console.log('      -> Pas de "projects" dans ce programme ou format invalide.');
          return;
        }

        program.projects.forEach(proj => {
          console.log(`      -> Projet: "${proj.projectName || proj.id}" (ID: ${proj.id})`);

          // Vérifier si le userId se trouve dans proj.participants
          // (Ici, participants est un tableau de userIds)
          if (proj.participants && proj.participants.includes(userId)) {
            console.log(`         [OK] L'utilisateur "${userId}" est participant dans ce projet.`);
            ...
            console.log(`         [NO] L'utilisateur "${userId}" n'est pas dans proj.participants.`);
          }
        });
      });
    });
