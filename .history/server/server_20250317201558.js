...
app.get('/api/pending-companies', async (req, res) => {
  try {
    ...
    // 🔍 Ajouter le `pseudo` du créateur à chaque entreprise
    for (let company of pendingCompanies) {
      const creator = await User.findOne({ username: company.userId }); // Trouver le créateur dans la DB `users`
      company.userPseudo = creator ? creator.pseudo : company.userId; // Ajouter le `pseudo` au lieu de `userId`
    }
    ...
    pendingCompanies = pendingCompanies.map(company => {
      ...
      return {
        ...
        userPseudo: company.userPseudo // 🔥 Ajout du pseudo du créateur
      };
    });
