...
app.get('/api/company/:companyName/members', async (req, res) => {
  try {
    ...
    // Rechercher la compagnie par son nom (en supposant qu'il est unique)
    const company = await ProjectCompany.findOne({ companyName });
    if (!company) {
      console.log('[SERVER] Aucune compagnie trouvée avec le nom:', companyName);
      return res.status(404).json({ message: 'Entreprise non trouvée.' });
    }
