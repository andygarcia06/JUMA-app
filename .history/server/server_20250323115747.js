...
app.get('/api/user/:userId/most-viewed-modules', async (req, res) => {
  try {
    ...
    // Rechercher l'entreprise dans ProjectManagement dont la liste des membres contient l'utilisateur
    ...
    // Récupérer les modules associés à cette société
    // Ici, on suppose que le champ "company" dans le modèle Module correspond au nom de la société.
    // Sinon, adaptez la requête (par exemple, filtrer par "creator.userId" ou autre critère).
    const modules = await Module.find({ company: company.companyName });
    
    // Pour chaque module, calculer le nombre total de réactions en parcourant ses cours
    const modulesWithReactions = modules.map(module => {
      ...
      return {
        id: module.id,
        title: module.title,
        courses: module.courses, // On renvoie la liste des cours pour permettre l'ouverture d'une modal
        reactions: totalReactions
      };
    });

    // Trier les modules par ordre décroissant du nombre total de réactions
    ...
    console.error('[SERVER] Erreur lors de la récupération des modules les plus consultés :', error);
