...
// Companies 
const Company = require('./schema/Company');
const ProjectManagement = require('./schema/ProjectManagement'); // Importez votre modèle de gestion de projet

app.get('/api/pending-companies', async (req, res) => {
  try {
    // Récupérer les entreprises validées : ici on considère celles où pendingValidation est false
    let companies = await Company.find({ pendingValidation: false });

    // Charger les données de gestion de projet pour récupérer les membres (si disponibles)
    let projectCompanies = await ProjectManagement.find();

    // Fusionner les membres de ProjectManagement dans chaque entreprise
    companies = companies.map(company => {
      const projectCompany = projectCompanies.find(proj => proj.id === company.id);
      return {
        ...company.toObject(),
        // Si projectCompany est trouvé, on prend ses membres, sinon on garde les membres déjà stockés (ou un tableau vide)
        members: projectCompany ? projectCompany.members : company.members || []
      };
    });

    console.log("✅ Données envoyées après fusion :", JSON.stringify(companies, null, 2));
    res.json(companies);
  } catch (error) {
