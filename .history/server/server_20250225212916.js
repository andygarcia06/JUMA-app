...
app.get('/api/user/:userId/progression', async (req, res) => {
    ...
    try {
        ...
        // Lire les modules depuis le fichier JSON
        const modulesData = JSON.parse(fs.readFileSync(MODULES_FILE_PATH, 'utf8'));

        let totalCourses = 0;
        let validatedCourses = 0;

        userProgress.modules.forEach(userModule => {
            validatedCourses += userModule.validatedCourses.length;

            const moduleFound = modulesData.find(m => m.id === userModule.moduleId);
            if (moduleFound) {
                totalCourses += moduleFound.courses.length;
            }
        });

        const progression = totalCourses > 0 ? (validatedCourses / totalCourses) * 100 : 0;

        res.json({ progression, validatedCourses, totalCourses });
    } catch (error) {
