...
app.post('/api/users/:userId/update-progression', async (req, res) => {
  try {
    ...
    // Rechercher l'utilisateur par son userId (ici, le champ "userId" dans le document)
    ...
    // Calculer la progression de chaque module pour cet utilisateur
    const modulesProgress = modules.map(module => {
      // On considère que user.validatedCourses contient les identifiants des cours validés
      const validatedCourses = user.validatedCourses
        ? user.validatedCourses.filter(courseId => 
            module.courses.some(course => course.id === courseId)
          )
        : [];
    ...
    // Mettre à jour ou créer l'entrée de progression dans la collection UserProgress
...
// Endpoint pour récupérer la progression de l'utilisateur depuis MongoDB
app.get('/api/users/:userId/progression', async (req, res) => {
  try {
    const { userId } = req.params;

    // Récupérer la progression de l'utilisateur depuis MongoDB
    const userProgress = await UserProgress.findOne({ userId });
    if (!userProgress) {
      return res.status(404).json({ message: 'Progression non trouvée pour cet utilisateur.' });
    }

    // Calculer la progression globale (pour affichage, par exemple)
    let totalCourses = 0;
