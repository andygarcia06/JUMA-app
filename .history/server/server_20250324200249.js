...
app.get('/api/company/:companyName/members', async (req, res) => {
  try {
    ...
    console.log(`[SERVER] companyName reçu dans l'URL: "${companyName}"`);
    ...
    console.log('[SERVER] Documents complets dans la DB:', allCompanies);

    // Tester le regex sur chaque compagnie
    allCompanies.forEach(c => {
      const regexTest = new RegExp(`^${companyName.trim()}$`, "i");
      const match = regexTest.test(c.companyName.trim());
      console.log(`[SERVER] Test regex: "${companyName.trim()}" vs "${c.companyName.trim()}" =>`, match);
    });
