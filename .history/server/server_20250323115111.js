...
// Importation du modèle ProjectManagement (assure-toi qu'il est bien exporté depuis schema/ProjectManagement.js)
const ProjectManagement = require('./schema/ProjectManagement');

// Route pour récupérer les modules les plus consultés pour l'entreprise de l'utilisateur
app.get('/api/user/:userId/most-viewed-modules', async (req, res) => {
  try {
    const { userId } = req.params; 
    console.log('[SERVER] Requête pour les modules les plus consultés pour userId:', userId);

    // Rechercher la société à laquelle appartient l'utilisateur dans ProjectManagement.
    // On suppose que dans ProjectManagement, le champ "members" est un tableau d'objets contenant, par exemple, { userId, email }
    const company = await ProjectManagement.findOne({ "members.userId": userId });
    if (!company) {
      return res.status(404).json({ message: 'Société non trouvée pour cet utilisateur.' });
    }
    console.log('[SERVER] Société trouvée:', company.companyName);

    // Ensuite, on récupère les modules associés à cette société.
    // Par exemple, si chaque module comporte un champ "company" ou si le créateur du module (creator.userId) correspond à l'entreprise,
    // on peut filtrer ainsi. Supposons que dans Module, il y a un champ "company" qui correspond au nom de la société.
    const modules = await Module.find({ company: company.companyName });
    // Si ce champ n'existe pas, on peut utiliser une autre logique (par exemple, le créateur du module).
    // const modules = await Module.find({ "creator.userId": company.userId });

    // Calculer le nombre total de réactions pour chaque module (en parcourant les cours et leurs réactions)
    const modulesWithReactions = modules.map(module => {
      ...
      return {
        id: module.id,
        title: module.title,
        courses: module.courses, // On renvoie les cours pour pouvoir les afficher dans la modal
        reactions: totalReactions
      };
    });

    // Trier les modules par ordre décroissant de réactions
    modulesWithReactions.sort((a, b) => b.reactions - a.reactions);

    console.log('[SERVER] Modules les plus consultés:', modulesWithReactions);
    res.status(200).json(modulesWithReactions);
  } catch (error) {
    console.error('Erreur lors de la récupération des modules les plus consultés :', error);
