...
const CompanyMembers = ({ companyId, userId }) => {
  ...
  useEffect(() => {
    ...
    const fetchUserOwner = async () => {
      try {
        ...
        if (company) {
          setUserOwner(String(company.userId).trim()); // 🔥 Convertir en chaîne de caractères
  ...
  console.log("🔍 User Actuel :", String(userId).trim());

  // ✅ Comparaison normalisée
  const isOwner = String(userId).trim() === userOwner;
