...
const CreateModuleReward = ({ userId }) => {
  ...
  // ^ Pour se souvenir du dernier niveau envoyé au serveur
  ...
  // ------------- Envoi automatique du badge débloqué au serveur -------------
  useEffect(() => {
    // Si highestBadge.name n'est pas vide et qu'il est différent du dernier niveau stocké
    if (highestBadge.name && highestBadge.name !== currentCreationLevel) {
      ...
      .then(res => {
        ...
        // Mettre à jour l'état pour ne pas rappeler l'API en boucle
