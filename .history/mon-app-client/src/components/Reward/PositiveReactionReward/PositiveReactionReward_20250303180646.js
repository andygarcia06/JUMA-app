...
const PositiveReactionReward = ({ userId }) => {
  ...
  // État local pour mémoriser le dernier badge envoyé
  ...
  // ------------------------------------------------------------------
  //   ENVOI AUTOMATIQUE DU BADGE DÉBLOQUÉ (positiveReactionsLevel)
  // ------------------------------------------------------------------
  useEffect(() => {
    // Si highestBadge.name n'est pas vide ET différent du dernier niveau stocké
    if (highestBadge.name && highestBadge.name !== currentPositiveLevel) {
      axios.post(`/api/users/${userId}/achievements`, {
        positiveReactionsLevel: highestBadge.name
      })
      .then(res => {
        console.log("Niveau de réactions positives mis à jour :", res.data);
        // Mettre à jour l'état pour éviter de renvoyer en boucle la même info
        setCurrentPositiveLevel(highestBadge.name);
      })
      .catch(err => {
        console.error("Erreur lors de la mise à jour du niveau de réactions positives :", err);
      });
    }
  }, [highestBadge.name, currentPositiveLevel, userId]);
