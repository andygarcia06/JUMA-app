...
const TicketList = ({ companyName, user, programId, context = "default" }) => {
  ...
  const getMeteoColor = (ticketId) => {
    const meteo = ticketMeteo[ticketId] || "Indéterminée";
    switch (meteo) {
      case "☀️ Positive":
        return "ticket-positive"; // Classe CSS pour vert
      case "🌤 Neutre":
        return "ticket-neutre"; // Classe CSS pour jaune
      case "🌧 Négative":
        return "ticket-negative"; // Classe CSS pour rouge
      default:
        return "ticket-indeterminee"; // Classe CSS pour gris
    }
  };
      ...
      <div className="ticket-section">
        ...
        {filterTickets(assignedOrSubscribedTickets).length > 0 ? (
          filterTickets(assignedOrSubscribedTickets).map(ticket => (
            <div key={ticket.id} className={`ticket-card ${getMeteoColor(ticket.id)}`} onClick={() => handleTicketClick(ticket.id)}>
              <span className="ticket-user">{ticket.userId}</span>
              <h5>{ticket.title || "Sans titre"}</h5>
              <p>{ticket.detail || "Pas de détails"}</p>
              <span className="ticket-status">{ticket.pendingValidationTicket}</span>
