...
const TicketList = ({ companyName, user, programId, context = "default" }) => {
  ...
  const handleTicketClick = (ticketId) => {
    ...
    navigate(`/ticket-entry/${ticketId}`, { state: { user } });
  };

  const getTicketBackgroundColor = (meteo) => {
    if (!meteo) return "grey-background";
  ...
  const isOpen = (ticket) => {
    ...
    return new Date(ticket.creationDate).toDateString() === today &&
  ...
  // Fonction de filtrage des tickets avec des comparaisons de dates en format ISO
