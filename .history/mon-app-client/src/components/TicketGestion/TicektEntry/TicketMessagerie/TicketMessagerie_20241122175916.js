...
const TicketMessagerie = ({ ticketId, userId }) => {
  ...
  const handleOpenModuleEntryPopup = (message) => {
    if (message.moduleId) {
        // Si moduleId est disponible, récupérez directement les détails du module
        console.log('ID du module trouvé dans le message :', message.moduleId);

        axios
            .get(`http://localhost:3001/api/tickets/modules/${message.moduleId}`) // API pour récupérer le module par ID
            .then((response) => {
                const moduleData = response.data;
                setPopupModuleContent(moduleData.content); // Afficher le contenu du module
                setPopupModuleId(moduleData.id); // Stocker l'ID pour référence
        ...
        // Fallback : si moduleId n'est pas disponible, utilisez le contenu
        const dynamicContent = message.content.replace('Module ajouté: Cours sélectionné :', '').trim();

        console.log('Recherche par contenu dynamique :', dynamicContent);

        axios
            .get(`http://localhost:3001/api/tickets/modules/search`, {
                params: { content: dynamicContent }, // Recherche par contenu
            })
            .then((response) => {
                const moduleData = response.data;
                setPopupModuleContent(moduleData.content);
                setPopupModuleId(moduleData.id);
                setShowModuleEntryPopup(true);
            })
            .catch((error) => {
                console.error('Erreur lors de la récupération du module par contenu :', error);
            });
    }
};
  ...
  return (
    <div className="ticket-messages-container">
      ...
      {showModuleEntryPopup && (
        <ModuleEntryPopup
          content={popupModuleContent} // Contenu du module
          moduleId={popupModuleId} // ID du module
