...
import natural from 'natural'; // 📌 Librairie NLP pour la comparaison de texte
...
const TicketMessagerie = ({ ticketId, userId }) => {
  ...
  const [dbEntries, setDbEntries] = useState({ positif: [], neutre: [], negatif: [] });
  const [similarityScores, setSimilarityScores] = useState({ positif: 0, neutre: 0, negatif: 0 });
  ...
  const fetchDbEntries = async () => {
    try {
      const response = await axios.get("http://localhost:3001/api/enrich-db");
      setDbEntries(response.data);
    } catch (error) {
  ...
  const analyzeProjectMeteo = () => {
    let scores = { positif: 0, neutre: 0, negatif: 0 };
    ...
    if (totalComparisons === 0) {
      setProjectMeteo("🌤 Indéterminée");
    } else {
      const dominantCategory = Object.keys(scores).reduce((a, b) =>
      ...
      setProjectMeteo(meteo);
      setSimilarityScores(scores);
    }
  };
  
  useEffect(() => {
    if (messageList.length > 0 && Object.keys(dbEntries).length > 0) {
      analyzeProjectMeteo();
    }
  }, [messageList, dbEntries]);
  ...
  return (
    <div className="ticket-messages-container">
      ...
      <div className="project-meteo">
        ...
        <button onClick={analyzeProjectMeteo}>🔄 Mettre à Jour</button>
      </div>
