  ...
  const handleOpenModuleEntryPopup = (message) => {
    axios
        .get(`http://localhost:3001/api/tickets/modules/search`, {
            params: { content: message.content }, // Passer le contenu du message
        })
        .then((response) => {
            const moduleData = response.data; // Récupère l'ID et le contenu du module
            setPopupModuleContent(moduleData.content); // Contenu récupéré
            setPopupModuleId(moduleData.id); // ID récupéré
            setShowModuleEntryPopup(true); // Affiche la popup
        })
        .catch((error) => {
