...
// Utilisation de useEffect pour récupérer les messages à chaque fois que le ticketId ou userId change
useEffect(() => {
  console.log(`Ticket ID reçu dans TicketMessagerie: ${ticketId}`);
  console.log(`User ID reçu dans TicketMessagerie: ${userId}`);

  // Vérification si ticketId et userId sont valides
  if (!ticketId || !userId) {
      console.error('Ticket ID ou User ID manquant');
      return;
  }

  // Récupérer les messages du ticket depuis l'API
  axios.get(`http://localhost:3001/api/messages/${ticketId}`)
      .then(response => {
          console.log('Messages récupérés avec succès:', response.data);
          setMessageList(response.data.messages);  // Mettre à jour l'état des messages
      })
      .catch(error => {
  ...
  // Connexion WebSocket pour les nouveaux messages
  connectWebSocket();
  ...
  const handleOpenModuleEntryPopup = (message) => {
    ...
    axios
        .get(`http://localhost:3001/api/messages/${messageId}`)
        .then((response) => {
...
const fetchModuleStats = (input) => {
  ...
  return (
    ...
    {messageList.length > 0 ? (
      ...
      <ModuleEntryPopup
          content={popupModuleContent} // Contenu du message
