...
const TicketMessagerie = ({ ticketId, userId }) => {
  ...
  const handleOpenModuleEntryPopup = (message) => {
    if (message.content.includes('Module ajouté:')) {
      console.log('Ouverture de la popup pour le module ID :', message.id); // Log pour vérifier
      setPopupModuleContent(message.content); // Contenu du module
      setPopupModuleId(message.id); // Stocke l'ID du module
