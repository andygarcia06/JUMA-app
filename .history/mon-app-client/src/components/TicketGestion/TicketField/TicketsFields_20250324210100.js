...
const TicketFields = () => {
  ...
  // L'objet ticket initial, en utilisant le pseudo de l'utilisateur et en initialisant la date en format ISO pour Mongoose
  const [ticket, setTicket] = useState({
    ...
    ticketNumber: '',
    title: '',
    organization: organization || '', // Pré-rempli avec l'organisation
    request: '',
    ...
    userId: user ? user.userId : '', // Utilisation de l'userId (vous pouvez adapter si besoin)
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        if (context === "company") {
          // Pour le contexte "company", on récupère toutes les compagnies et on trouve celle dont le companyName correspond à l'organisation
          ...
          // Pour le contexte "program", on récupère les participants via une route dédiée
  ...
  // Gestion des changements du champ Subscribers (sélection multiple)
