...
const ProjectTab = ({ companyId, programId, projectId, tabId, tabName, userId }) => {
  ...
  const handleAddRow = async () => {
    ...
    if (!newRow.rowName || !newRow.goal || !newRow.priority || !newRow.type || newRow.budget === 0 || newRow.actual === undefined || !newRow.status) {
    ...
    const payload = {
      ...newRow,
      tabId,       // ID de la tab
      tabName,     // Nom de la tab (vérifiez sa valeur ici)
      projectId,   // ID du projet
      programId,   // ID du programme
      companyId,   // ID de la compagnie
      userId       // ID de l'utilisateur (propriétaire de la row)
    };
  
    // Afficher le contenu de payload pour déboguer et vérifier les données envoyées
    console.log('Données envoyées pour ajouter une row:', payload);
  
    try {
      // Envoyer toutes les informations nécessaires dans la requête POST
