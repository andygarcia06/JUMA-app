...
const ProjectRows = ({ companyId, programId, projectId, tabId }) => {
  ...
  // Fonction pour gérer les changements de données du projet fonctionnel
  ...
  const handleSubmitFunctionalProject = async (e, rowId) => {
    e.preventDefault();
    try {
      await axios.post(`http://localhost:3001/tabs/${tabId}/rows/${rowId}/functionalProject`, functionalProjectData);
      alert('Projet fonctionnel ajouté avec succès');
                ...
                <tr>
                  <td colSpan="10">
                    <div className="project-under-row">
                      <form onSubmit={(e) => handleSubmitFunctionalProject(e, row.rowId)}>
