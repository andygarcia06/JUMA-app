...
const ProjectRows = ({ companyId, programId, projectId, tabId }) => {
  ...
  // Récupérer les rows depuis le serveur
  useEffect(() => {
  ...
  const handleChangeFunctionalData = (e) => {
    const { name, value } = e.target;
    setFunctionalProjectData((prev) => ({
      ...prev,
      [name]: value
    }));
  };

  // Soumission du formulaire pour ajouter un projet fonctionnel
  ...
  // Ouverture/fermeture du formulaire pour ajouter un projet fonctionnel
      ...
      <div className="add-row-form">
        ...
        <form onSubmit={handleAddRow}>
          <div>
            ...
            <input type="text" name="rowName" value={newRowData.rowName} onChange={handleNewRowChange} required />
          </div>
          <div>
            <label>Propriétaire :</label>
            <input type="text" name="owner" value={newRowData.owner} onChange={handleNewRowChange} required />
          </div>
          <div>
            <label>Objectif :</label>
            <input type="text" name="goal" value={newRowData.goal} onChange={handleNewRowChange} required />
          </div>
          <div>
          ...
          <div>
            <label>Type :</label>
            <input type="text" name="type" value={newRowData.type} onChange={handleNewRowChange} required />
          </div>
          <div>
            <label>Budget :</label>
            <input type="number" name="budget" value={newRowData.budget} onChange={handleNewRowChange} required />
          </div>
          <div>
            <label>Réel :</label>
            <input type="number" name="actual" value={newRowData.actual} onChange={handleNewRowChange} required />
          </div>
          <div>
            <label>Statut :</label>
          ...
          {rows.map((row) => (
            <React.Fragment key={row.rowId}>
              <tr>
                ...
                <td
                  style={{
                    backgroundColor: row.status === 'Terminé' ? 'green' : (row.status === 'En cours' ? 'yellow' : 'gray'),
                ...
                >
                  <select
                    ...
                    style={{ border: 'none', borderRadius: '5px', padding: '5px', width: '100px' }}
                  >
                    <option value="En cours">En cours</option>
                    <option value="Terminé">Terminé</option>
                    <option value="Null">Null</option>
              ...
              {/* Formulaire pour ajouter un projet fonctionnel à la ligne sélectionnée */}
