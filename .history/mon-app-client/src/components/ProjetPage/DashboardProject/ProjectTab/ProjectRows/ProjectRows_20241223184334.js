...
const handleSubmitFunctionalProject = async (e) => {
  ...
  try {
    ...
    if (!projectId) {
      console.error("Le projectId n'est pas défini.");
      return;
    }

    // Soumettre la requête avec les données nécessaires
    const response = await axios.post(`http://localhost:3001/projects/${projectId}/functional`, {
      companyId,
      programId,
      tabId,
      ...functionalProjectData // Ajouter les données du projet fonctionnel
    });
