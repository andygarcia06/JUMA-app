...
const DashboardProject = ({ companyId, userId, programId, projectId, programName, companyName }) => {
  ...
  const saveNewTab = async (tab) => {
    try {
      const response = await axios.post(`/projects/${projectId}/tabs`, {
        companyId,
        programId,
        projectId,
        tabId: tab.id,
...
{loading ? (
  ...
  <>
    {/* Afficher les noms des tabs */}
    <h3>Liste des Tabs :</h3>
    {projectTabs.length > 0 ? (
      <ul>
        {projectTabs.map((tab) => (
          <li key={tab.tabId}>{tab.tabName}</li> // Utilisation de "tabName" au lieu de "name"
        ))}
      </ul>
    ) : (
