...
const Requests = () => {
  ...
  const fetchPendingCompanies = async () => {
    try {
      ...
      // Filtrer en comparant company.userId avec user.pseudo (car pour l'utilisateur "1", user.pseudo vaut "1")
  ...
  // Fonction de recherche des utilisateurs dans la BDD via un endpoint de recherche
  ...
  const handleAddMember = (member) => {
    ...
    // Optionnel : ne pas effacer la recherche pour permettre l'ajout multiple sans ressaisir
    // Vous pouvez décider de réinitialiser si vous préférez :
    // setSearchQuery('');
    // setUserResults([]);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post('/api/pending-companies', {
        companyName,
        description,
        // Utiliser user.pseudo pour être cohérent avec la BDD
        userId: user.pseudo,
        category,
            ...
            {userResults.length > 0 && (
              <ul>
                {userResults.map((member) => (
                  <li key={member.userId} onClick={() => handleAddMember(member)}>
