...
const Requests = () => {
  ...
  const fetchPendingCompanies = async () => {
    try {
      ...
      // On filtre en comparant company.userId avec user.pseudo (parce que pour l'utilisateur "1", user.pseudo vaut "1")
  ...
  // Fonction de recherche des utilisateurs via l'endpoint /api/users/search
  ...
  // Utiliser l'identifiant unique _id pour éviter les doublons
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Créer la nouvelle entreprise sans membres
      const companyResponse = await axios.post('/api/pending-companies', {
        ...
        members: [] // On démarre sans membres
      });
      const newCompany = companyResponse.data;
      // Puis mettre à jour l'entreprise pour y ajouter les membres sélectionnés
      await axios.post(`/api/company/${newCompany.id}/members`, {
