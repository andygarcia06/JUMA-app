...
const NewKnowledgeItem = ({ onClose, onNewModule, onNewCourse, modules, user }) => {
  ...
  const handleNewModule = () => {
    if (title.trim() !== '') {
      const newModule = {
        title: title,
        createdAt: new Date(),
        creator: 'User', // Remplacer 'User' par l'utilisateur réel si besoin
      };
  ...
  const handleNewCourse = () => {
    // Remarque : on attend que le cours ait un champ "moduleId" et non "moduleName"
    if (title.trim() !== '' && moduleId.trim() !== '' && editorContent.trim() !== '') {
      const newCourse = {
        title: title,
        moduleId: moduleId,  // Correction ici : on utilise moduleId
  ...
  return (
    <div className="popup-container">
      <div className="popup">
        ...
        <button className='close-new-course' onClick={onClose}>X</button>
      </div>
    </div>
  );
};
