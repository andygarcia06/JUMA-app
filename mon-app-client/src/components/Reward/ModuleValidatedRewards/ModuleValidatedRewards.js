...
const ModuleValidateRewards = ({ userId }) => {
  ...
  // État local pour mémoriser le dernier niveau envoyé au serveur
  ...
  // -----------------------------------------
  //   ENVOI AUTOMATIQUE DU NIVEAU DÉBLOQUÉ
  // -----------------------------------------
  useEffect(() => {
    ...
    if (highestBadge.name && highestBadge.name !== currentValidatedLevel) {
      axios.post(`/api/users/${userId}/achievements`, {
        modulesValidatedLevel: highestBadge.name
      })
      .then(res => {
        console.log("Niveau de validation modules mis à jour :", res.data);
        // Mémoriser le niveau pour éviter de rappeler l'API en boucle
        setCurrentValidatedLevel(highestBadge.name);
      })
      .catch(err => {
        console.error("Erreur lors de la mise à jour du niveau de validation modules :", err);
      });
    }
  }, [highestBadge.name, currentValidatedLevel, userId]);
